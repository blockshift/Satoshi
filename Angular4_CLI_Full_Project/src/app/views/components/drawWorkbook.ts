...
export function drawWorksheet(workbook: wjcXlsx.IWorkbook, sheetIndex: number, rootElement: HTMLElement,
    maxRows, maxColumns) {
    ...
    if (sheet.columns) {
        ...
        for (var c = 0; c < maxRowCells; c++) {
			var col = c < columnCount ? sheet.columns[c] : null,
            ...
            for (var c = 0; row.cells && c < row.cells.length; c++) {
                ...
                if (cell) {
                    ...
                    if (!(value == null || value !== value)) { // TBD: check for NaN should be eliminated
                        ...
                        var fmtValue = netFormat ? wjcCore.Globalize.format(value, netFormat) : value;
...
function importStyle(cssStyle: CSSStyleDeclaration, xlsxStyle: wjcXlsx.IWorkbookStyle) {
    if (!xlsxStyle) {
        return;
    }
    if (xlsxStyle.fill) {
